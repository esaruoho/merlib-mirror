---
title: "RICHARD FEYNMAN - Nobel Lecture, December1965"
source_domain: radiondistics.altervista.org
source_path: richard_feynman_nobel_lecture.htm
order: 38
...
# RICHARD FEYNMAN - Nobel Lecture, December1965

*Source page: `richard_feynman_nobel_lecture.htm`*
...
I did gather from my readings, however,
 ...
 and there is an infinite number of modes in a box of every
...
independent
 degrees of freedom and the infinities from the
...
R
, between the charges, while the force of radiation
  ...
  charges the inverse 
R
 square would be compensated by the
  
R
2
 in the volume element and we would get a
...
Professor Wheeler used advanced waves to
  ...
  would be an index of refraction 
n
, so, the retarded waves
...
I found that you get the right answer if
  ...
  Suppose the source 
s
 surrounded by a spherical absorbing
...
t
=0 induces motions in the wall at time +10. Advanced
  effects from this can act on the test charge as early as eleven
  seconds earlier, or at 
t
= -1. This is just at the time
...
Thus, it became clear that there was the
  possibility that if we assume all actions are 
via
...
I
2
 between two points on the paths. Thus,
...
The fact that the interaction is exactly
  ...
  such a principle of least action, whereas interaction 
via
...
, 
I
 omit self-interaction and no longer
...
ij
  ...
  replace the delta function of 
I
2
 by a narrow
  peaked thing. Let's say that 
f
(
Z
) is large only
...
T
 is the time
  difference and 
R
 is the separation of the charges. This
  might look like it disagrees with experience, but if 
a
 is
  some small distance, like 10
-13
 cm, it says that the
  time delay 
T
 in action is roughly 
 or approximately, - if 
R
 is much larger than
...
T
 from the ideal theoretical
  time 
R
 of Maxwell, gets smaller and smaller, the further
  ...
  adequately satisfied if were 10
-13
 cm. If 
R
 is
  of the order of a centimeter this deviation in 
T
 is only
  10
-26
 parts. So, it was possible, also, to change the
  theory in a simple manner and to still agree with all
  observations of classical electrodynamics. You have no clue of
  precisely what function to put in for 
f
, but it was an
...
m
i
, term, all the mechanical mass could be
  ...
  action 
A
. In expression (1) only the second term is kept,
  the sum extended over all 
i
 and 
j
, and some
...
I would also like to emphasize that by this
  ...
  the equations obtained by variation (of X
i
m
 (
a
i
)) are no longer at all easy
...
an ordinary electron
 world line, in the section in which
  it 
reversed itself and is coming back
 from the future we have the
  wrong sign to the proper time - to the proper four velocities -
  
and that's equivalent to changing the sign of the charge
, and,
  therefore, that part of a path 
would act like a positron
."
 "But,
  ...
  electrons were the same one from him as seriously as 
I took the
  observation that positrons could simply be represented as
  electrons going from the future to the past
 in a back section of
...
So, the problem is only to make a quantum
  ...
  variables and replace them by 
, but I couldn't find a momentum variable, as there wasn't
  any.
...
The character of quantum mechanics of the
  ...
  operator, 
H
. If the classical physics could be reduced to
...
x
  ...
  had to put a constant of proportionality 
A
 in, suitably
  adjusted. When I substituted 
 for 
K
 to get
...
+
e
 and then I could substitute
  that back into (3) to get another factor of e
i
e
L
 and give me the wave function the
  next moment, 
t
+2
e
 and so on and
  ...
  exponential of the sum of terms like 
e
L
. Now, 
L
...
d
t
, you just take the value at each
  point and add them together. We are to take the limit as
  
e
-0, of course. Therefore, the
...
e
 goes to zero, of course) of exponential
  
 where 
S
 is the action expression (2). At
...
This led later on to the idea of the
  ...
  an amplitude. That amplitude is e to the 
 times the action for the path. Amplitudes from various
...
It was also easy to guess how to modify the
  ...
  changed the delta to an 
f,
 just as I would for the
  ...
  integrals in which there are two 
S
's and so forth. Yet, as
...
Lamb
 did his experiment,
  measuring the separation of the 2
S
½ and
  2
P
½ levels of hydrogen, finding it to be about 1000
  megacycles of frequency difference. Professor 
Bethe
, with whom I was
...
m
  ...
  for 
m
 in the results for any other problem and all these
...
After the lecture, I went up to him and
  ...
  the delta-function to 
f
 and asked him to explain to me how
...
a
 is the width of
  that function 
f
 which was substituted for 
d
. If you wanted an unmodified electrodynamics,
  you would have to take 
a
 equal to zero, getting an
  ...
  suggested, were not sensitive to 
a
 in this form and even
  had a definite limit as 
a
g
0.
...
The rest of my work was simply to improve
  ...
  be replaced by Dirac's matrix 
a
 or in
  the more relativistic forms by the operators 
. I just took my guesses from the forms that I had worked
...
During this time, people had been
  ...
  to the photon had a mass. It was easy to guess the 
d
-function in (1), which was a solution of
  d'Alembertian equals zero, was to be changed to the corresponding
  solution of d'Alembertian equals 
m
2
. Next,
...
, are called vector mesons - there were also scalar mesons.
...
Q
 in there, that variable
  
Q
?" (I had expressions like (tan 
-1
Q
)
  /
Q
 etc.). I said, "that's the momentum transferred by the
  electron, the electron deflected by different angles." "Oh", he
  said, "no, I only have the limiting value as 
Q
 approaches
  zero; the forward scattering." Well, it was easy enough to just
  substitute 
Q
 equals zero in my form and I then got the
...
a
2
, so that I could calculate finite
...
f
 the calculations would give results which were not
  ...
  small, in practice, if 
a
 was very small. In the limit that
  I took 
a
 very tiny, it might not make any difference. And,
...
It is lucky that I did not wait to
  ...
  understanding the 
strong
 interactions is that there isn't
...
best
 or most efficient method to proceed to a discovery,
