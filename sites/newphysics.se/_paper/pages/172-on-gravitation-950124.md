---
title: "ON GRAVITATION 950124"
source_domain: newphysics.se
source_path: teden/Tedenstig/ON GRAVITATION 950124.html
order: 172
...
*Source page: `teden/Tedenstig/ON GRAVITATION 950124.html`*

ON GRAVITATION
( Revised 12/8-1994)
( Revised 24/1 -1995, the input rate R is some larger than H 
  because of exponential growth in mass )
...
finaly solved is perhaps that the gravity process is of a
very complex nature and where so many other things in fundamental
physics are involved in its final solution.

Our basic idea is that the force emanates from a free thermal
...
an approximate value of 3x10-18 s-1  . Also involved as a base
...
from the free thermal field which directly causes the gravity
...
We have to take consideration to that the mass of the body
grow all the time, hence the growth is not exactly liniar
but exponential. The real inflow rate counted over a long
time therefore will be larger than H. This factor can for
instance be computed by the following computer program :

                   M:=m+dm
                   t:=t+dt
                   dm:=M*dt/T
                   H:= 1/T
we find that H=0.69 times R or R= H/0.69;  
...
Using the results of (16,17) gives :
           ...
           2                  2                            2
F = qD.A2.v = M1.R/(v.Ad).A2.v  = M1.R/(v.Ad).(M2/mp)).Ap.v =

            2
    (M1.M2/D ).(R.Ap.v/(4.Pi.mp))       or

19) ===================================
                2
    F = (M1.M2/D ). (R.Ap.v/(4.Pi.mp)
...
20) ====================================================
    ...
    G = (R*Ap*v/(4*Pi*mp)) = (H/0.69 * Ap*v/(4*pi*mp))

========================================================

The proton, being of torus form, having a radius of Rp  give
the proton interacting area equal to Ap = 2.Pi.Rp.2.P.Rp,
giving :
...
(assumly this change is not exactly liniar over long time )
...
(7) A new model of interaction between matter and vacuum,
    Ove Tedenstig, Galilean Electrodynamics 1993.

(Earlier published in Toth Maatian Review)
