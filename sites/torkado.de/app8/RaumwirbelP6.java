...
public class RaumwirbelP6 extends Applet implements ActionListener, Runnable
{
...
//----------------------------------------------------------
 ...
 for (ii = 0; ii < ianz; ii++ )
 {
   ...
   ph1 += P120;
    ...
    if(Math.abs(arg) < 50)
    { paramv = neunA*( - Math.pow(fiGold,arg)); }
  ...
  while (is > -Nkreisteile)
  {
         ...
         //delta = 1.0-Math.pow(2.0, winkelphi);
         //neunC = Eins + neunB*(1.0-Math.pow(2.0,delta));
