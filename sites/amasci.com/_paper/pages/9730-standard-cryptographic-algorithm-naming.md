...
source_path: ~weidai/scan-mirror/kf.html
order: 9730
...
*Source page: `~weidai/scan-mirror/kf.html`*
...
- \[*Def*\] PKCS \#8: Private-Key Information Syntax Standard,\
  ...
  <a href="ftp://ftp.rsa.com/pub/pkcs/ascii/pkcs-8.asc" target="_blank">ftp://ftp.rsa.com/pub/pkcs/ascii/pkcs-8.asc</a>
...
<span id="Generic/Base64-ASN1"></span>
...
**Description:**  
This factory uses the same underlying encoding as [Generic/ASN.1](#Generic/ASN1), but converted to a restricted set of ASCII characters using Base64 (RFC 2045).
...
**References:**  
- \[see references for [Generic/ASN.1](#Generic/ASN1)\]
...
- A series of multi-precision integers comprising the key material. This algorithm-specific portion is:
  ...
  - MPI of Elgamal public key value *y* (= *g<sup>x</sup>* where *x* is secret).
...
**Comments:**  
- Since OpenPGP DH keys do not contain a specification of the order of the subgroup generated by *g*, the key SHOULD NOT have an `order` parameter. This will result in the exponent *k* used for encryption being chosen from the range \[1, *p*-1\] (or possibly \[2, *p*-2\]), which matches the behaviour of existing OpenPGP implementations.
- Despite the existence of algorithm number 20, designated as "Elgamal (Encrypt or Sign)" in the OpenPGP spec, it is not guaranteed to be valid to use this KeyFactory algorithm to create a public or private key that will be used for Elgamal signatures ([ElgamalSig](sig.html#ElgamalSig)). Instead, use an [ElgamalSig/OpenPGP](#ElgamalSig/OpenPGP) KeyFactory.

**Security comment:**  
Use of the same key pair for Elgamal signatures and encryption is not recommended.

<span id="DSA/OpenPGP"></span>
...
- A series of multi-precision integers comprising the key material. This algorithm-specific portion is:
  ...
  - MPI of DSA public key value *y* (= *g<sup>x</sup>* where *x* is secret).
...
A trinomial basis over GF(2<sup>m</sup>) with reduction polynomial *x<sup>m</sup>* + *x<sup>k</sup>* + 1, is represented as the integer *k*:

       Trinomial ::= INTEGER

A pentanomial basis over GF(2<sup>m</sup>) with reduction polynomial *x<sup>m</sup>* + *x*<sup>*k*<sub>3</sub></sup> + *x*<sup>*k*<sub>2</sub></sup> + *x*<sup>*k*<sub>1</sub></sup> + 1, is represented as the integers *k*<sub>1</sub>, *k*<sub>2</sub> and *k*<sub>3</sub> (in order, with smallest first):
...
The ASN.1 types used in ECDSA keys are the same as those given above for [ECDH/ASN.1](#ECDH/ASN1). Output MUST be encoded as DER.
...
<span id="ElgamalSig/OpenPGP"></span>
...
**Comments:**  
- Despite the existence of algorithm number 20, designated as "Elgamal (Encrypt or Sign)" in the OpenPGP spec, it is not guaranteed to be valid to use this KeyFactory algorithm to create a public or private key that will be used for Elgamal encryption ([ElgamalEnc](ca.html#ElgamalEnc)). Instead, use an [DH/OpenPGP](#DH/OpenPGP) (alias ElgamalEnc/OpenPGP) KeyFactory.

**Security comments:**  
- The paper by Bleichenbacher referenced above shows that if *g* has only small prime factors, and if *g* divides the order of the group it generates, then signatures can be forged. Implementations MAY reject encoded keys that would be vulnerable to this attack (in Java, by throwing an InvalidKeySpecException).
- Use of the same key pair for Elgamal signatures and encryption is not recommended.
...
The bit length of *n* MUST be a multiple of 3. That is, there is an integer *k* such that 2<sup>3*k* - 1</sup> \< n \< 2<sup>3*k*</sup>. Also, *e* satisfies 8 \<= *e* \< *n* and is relatively prime to *n*.

For a private key, *n* = *p*<sup>2</sup>*q* and 2<sup>*k*-1</sup> \< *p*, *q* \< 2<sup>*k*</sup> MUST also hold.
