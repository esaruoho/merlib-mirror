...
source_path: ~weidai/scan-mirror/conventions.html
order: 9720
...
*Source page: `~weidai/scan-mirror/conventions.html`*
...
## <span id="interfaces">Algorithm Interfaces</span>

The inputs and outputs of algorithms (plaintext, ciphertext, keys, input messages, etc.) are normally sequences of bytes. However, in the case of some message digests the input may also be given as a sequence of bits, as explained <a href="md.html#interop" target="_blank">here</a>.
...
- in each pair, the hex digit on the left is most significant,
- the pairs are written from left to right, i.e. the first byte corresponds to the leftmost pair of hex digits.

Unless the specification of an algorithm describes any of its inputs or outputs in terms of bit sequences, the order of bits within a byte is not, and does not need to be defined. If you are designing an algorithm, suggestions about byte and bit order, among other issues, are given <a href="suggestions.html" target="_blank">here</a>.

## <span id="references">Categories for References</span>

References WILL be marked as being in one or more of the following categories:

- "\[*Def*\]" - defines the algorithm,
- "\[*Inf*\]" - gives additional information or an alternative description,
- "\[*History*\]" - describes an earlier variant of the algorithm,
- "\[*An*\]" - cryptanalysis (with either negative or positive implications for the algorithm's security),
- "\[*Patent*\]" - patents or patent applications that may affect implementations of the algorithm,
- "\[*Test*\]" - includes test vectors,
- "\[*Impl*\]" - includes a reference implementation, written or endorsed by the algorithm designer(s). Note that in many cases these implementations are not optimised.

Note that the \[*Patent*\] references cannot be guaranteed to include all applicable patents.

## <span id="security">Security implications</span>
...
There may be published attacks that are not listed in the security comments or references. If you know of any such attacks, or if you want to dispute something said in these comments, please post to [sci.crypt](news:sci.crypt).
...
## <span id="parameters">Algorithm Parameters</span>
...
## <span id="adding">Adding New Algorithms</span>

If you would like to add an entry to the list, please post it to the Usenet newsgroup [sci.crypt](news:sci.crypt), with a subject line that includes "SCAN". The post should give the text of the proposed entry, in the same format as other algorithms of the same type, and ideally should include:

- references to at least one paper or web page describing the algorithm (preferably, although not necessarily, a paper published in a refereed journal).
- any other useful references (cryptanalysis, home page, patents and patent applications, reference implementation, etc.)
- if the references do not completely specify the algorithm (including input/output formats, etc.) in sufficient detail to allow interoperable implementations, then any missing details should be given.
- test vectors, where available (unless it is particularly simple, an algorithm without test vectors WILL only be listed as provisional or experimental; see below).
...
## <span id="status">Algorithm status and stability</span>

Each SCAN entry (not including [alleged algorithms](#alleged)) is classified and colour-coded as one of:

 • <span class="s">Blue - stable</span>\
 ? <span class="p">Green - provisional</span>\
× <span class="x">Red - experimental</span>

The symbols '•', '?' and '×' give the same information as the colour-coding. (If you don't see blue, green and red text above, or if '•' or '×' display as boxes, see the [browser problems](#browsers) section below.)
...
## <span id="alleged">Alleged Algorithms</span>
...
- really more than one algorithm,
- a duplicate of another algorithm,
- not of the stated type,
- ambiguously specified,
- incorrectly stated to be broken,
- incorrectly stated to be related to another algorithm,
- incorrectly stated to be anything else,
- associated with incorrect or inconsistent test vectors,
- missing obvious references,
- linked to irrelevant references,
- completely obscure,
- inherently useless or uninteresting,
- unimplementable or impractical,
- a figment of my imagination,
- named differently to any future entry for that algorithm in SCAN,
...
## <span id="provisional">Provisional Algorithm Names</span>
...
## <span id="objectids">ASN.1 Object Identifiers</span>

Where there is an ASN.1 OBJECT IDENTIFIER for an algorithm, the dotted-decimal form of the identifier can be used implicitly as an alias. These aliases WILL be the only names that match '`oid`' from the following ABNF syntax (which is a subset of that given in Section 2.15 of <a href="http://www.rfc-editor.org/rfc/rfc1778.txt" target="_blank">RFC 1778</a>):
...
## <span id="identifiers">Names as Programming Language Identifiers</span>
...
An identifier satisfying this syntax can be derived for each basic name (not including parameters) as follows:

- If the entry contains an alias marked "(use for identifiers)", then use that alias.
- Otherwise, use the standard name with any "-" characters replaced by "\_".

New names and aliases WILL be chosen so that this procedure always works, and is reversible (by replacing "\_" by "-") to give a valid alias.

## <span id="ciphers">Conventions for symmetric ciphers</span>

The information that was in this section has moved to the [symmetric ciphers page](csi.html).

## <span id="browsers">Browser problems</span>

To see SCAN as intended:

- Make sure that your browser preferences are not set to override colours and styles set by the document. In Netscape or Mozilla, select "Edit menu \| Preferences..." and look at the "Appearance -\> Colors" tab. In MSIE, select "Tools menu \| Internet Options..." and click the button labelled "Accessibility..." on the "General" tab.
- If you still cannot see the colour coding and/or symbols, you will need to upgrade to a browser that supports Unicode and Level 1 Cascading Style Sheets. (The minimal and bug-ridden support provided by Netscape 4.x or MSIE 3.x is sufficient.)

## <span id="viewers">Viewing PDF and Postscript files</span>

Most of the on-line references are in PDF and/or PostScript format. Versions of the Acrobat Reader<sup><span class="small">TM</span></sup> PDF viewer for several operating systems are available from Adobe's site:

> <a href="http://www.adobe.com/products/acrobat/alternate.html" target="_blank"><img src="getacro.gif" data-border="0" width="88" height="31" alt="Get Acrobat Reader" /></a>

To view PostScript files, you will need GNU or Aladdin Ghostscript, and either Ghostview or GSview, all of which are available from <a href="http://www.cs.wisc.edu/~ghost/" target="_blank">here</a>. Note that in at least some versions of Netscape, compressed PostScript files (with filetypes ending .ps.Z or .ps.gz), seem to be downloaded incorrectly when the link to a file is clicked on directly. If you encounter this bug, it can be worked around by right-clicking on the link, selecting "Save Link As..." to save the compressed file, and decompressing it manually (on Windows, use a decompression utility that understands .Z and .gz formats, such as <a href="http://www.winzip.com" target="_blank">Winzip</a>).

Alternatively, the <a href="http://wheel.compose.cs.cmu.edu:8001/cgi-bin/browse/objweb" target="_blank">TOM Conversion Server</a> is a very useful service for converting documents between different formats (you may find that the file upload option works if the URL conversion option does not).
...
[Copyright and trademark information](intro.html#copyright)
