---
title: "A Basic Win32 GUI Program--WINBASIC.ASM"
source_domain: amasci.com
source_path: ~htak/win32asm/winbasic.htm
order: 3465
...
*Source page: `~htak/win32asm/winbasic.htm`*

# A Basic Win32 GUI Program--WINBASIC.ASM

This program produces a basic application window. You can move the window around. You can change the window's size, and all the usual buttons appear and are functional. The only things missing are scroll bars and a menu bar.

- [Program startup](#startup)
- [The window class](#winclass)
- [Window creation](#wincreate)
- [The message loop](#msgloop)
- [Terminating the program](#terminate)
...
\[[More on program startup.](hardcore.htm#startup)\]
...
### <span id="winclass"></span>The window class
...
The lpfnWndProc field contains the address of the ***[window procedure](#wndproc)*** that defines how the windows of this class will behave.\
...
### <span id="wincreate"></span>Window creation
...
### <span id="msgloop"></span>The message loop
...
### <span id="terminate"></span>Terminating the program
...
### <span id="wndproc"></span>The window procedure
...
### <span id="linking"></span>Linking

Linking a standard application is similar to linking a Console App. With the Microsoft linker, we specify a different [subsystem](hardcore.htm#subsystem).

        link winbasic kernel32.lib user32.lib /entry:start /subsystem:windows

With the Borland linker, we specify a standard Windows app with /aa instead of /ap. The /V option tells the linker to set the [subsystem version](hardcore.htm#subsystem) to 4.0. (Warning: The linker that came with TASM 4.0 does not support /V.)
