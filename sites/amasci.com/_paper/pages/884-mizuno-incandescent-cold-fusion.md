---
title: "Mizuno Incandescent Cold Fusion"
source_domain: amasci.com
source_path: weird/anode.html
order: 884
...
# Mizuno Incandescent Cold Fusion

*Source page: `weird/anode.html`*
...
- [Replication by Little/Puthoff/Little](http://www.earthtech.org/experiments/Inc-W/Mizuno.html)
- [Naudin's test](http://jlnlabs.online.fr/cfr/html/cfr10.htm)
- [CF by plasma electrolysis](http://guns.connect.fi/innoplaza/energy/story/Kanarev/coldfusion/)
- [IE device update](http://www.infinite-energy.com/iemagazine/issue26/deviceupdate.html)
...
I've seen an effect VERY like the stuff Mizuno is working with. It's called "the glowing pickle" demonstration, and is used as a sort of humerous lecture demo in physics classes. During the demo a pair of nails is inserted into a pickle... then is connected directly to 120VAC!!!! Yellow-colored electric arcs develop in the small gap between the metal surface and the salty pickle flesh. Similar effects occur if the pickle is replaced with a block of agar filled with salt solution.\

From observing "glowing pickle" I have some notes and suggestions. First, the blue/violet color in the early stages of Mizuno's phenomenon is almost certainly caused by emission lines of potassium, since the "pickle" experiment gives off brilliant sodium-yellow light. Change the salt, and the emission color should change. Try strontium salts for red!\

Second: in the pickle experiment, the Liedenfrost effect appears as the metal electrodes heat up: the water cannot touch the metal because it instantly generates H2O gas (live steam.) But in this case the steam, as well as other ions from the electrolyte and the electrodes, participates in an electric arc. If the electrode is hot enough, there will be a constant gas-filled gap surrounding the metal. This being the case, the experiment could be sped up by using a blow-torch to pre-heat the centrl metal electrode to white hot incandescence before inserting it into the electrolyte.\

Too bad it's so difficult to perform experiments under weightless conditions. It would be very interesting to observe this system with convection removed from the picture. The gas-layer would not be swept upwards as steam bubbles. What would happen? Would the gas layer grow larger and the arc be quenched? Perhaps some thixotropic substance could damp out the convection (replace the electrolyte with a gel?)\

Another variant: replace the small electrode with a large one, separate the electrolyte into two insulated portions, then strike the arc between fluid jets so the metal electrodes don't contaminate the arc plasma at all.\

Another: Mizuno's regulated DC... is it required? What happens if AC is used? Perhaps the system could be inexpensively powered directly from 120VAC mains (or use a variac if other voltages are needed.)\

[/weird/anode.html](/weird/anode.html)\
