...
void setup() 
{
  ...
  world[0]=new Particle(0,0,0,0,130.0);

  world[1]=new Particle(18,0,0,0.59,0.5);
  world[2]=new Particle(-35,0,0,-0.5,1.0);
  world[3]=new Particle(39.654,0,0,0.4,1.0);

  /*
  for(int i=1; i<n; i++){
    world[i] = new Particle(boxmuller()*100,boxmuller()*100,random(-1,1)*0.9,random(-1,1)*0.9,1.0*random(1,4));
  }
  */
}
...
class Particle
{
  ...
  void draw(){
    ...
    x = floor((px-world[0].px)*4)+width/2;
    y = floor((py-world[0].py)*4)+height/2;
